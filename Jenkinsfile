...
pipeline {
  ...
  stages {
    ...
    stage('Package') {
      steps {
        sh "nuget4mono -g origin/${env.BRANCH_NAME} -p ${workspace}/Terradue.Cloud/packages.config ${workspace}/Terradue.Cloud/bin/Terradue.Cloud.dll ${workspace}/Terradue.Cloud/Resources/**/*,content/modules/cloud"
        sh 'cat *.nuspec'
